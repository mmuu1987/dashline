...
- **积木库（core.4 已实现）**：平地 / 三档宽坑（按跳跃能力比例取宽）/ 尖刺簇（1–3 连）/ 浮空台阶 / 奖励拱弧，以及六种机制积木：
  ...
  - 🛗 **升降电梯**（35% 后解锁）：刺毯上方三角波升降台（纯 tick 函数驱动，跨引擎逐位一致）——乘坐稳过拿高处宝石，或点按连跳硬闯；
  - ⚡ **加速带**（55% 后解锁）：踩上获得 1.5s ×1.55 提速 → 冲上超过满蓄力射程的超远坑，无加速物理不可能；
  - 💍 **二段跳环**（55% 后解锁）：超宽坑中央弧线顶点走廊悬环，空中穿环获一次额外空中跳接力过坑；起跳点前后漂移 ~60px 仍可吃到。
